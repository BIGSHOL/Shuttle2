...
// KIDS 모드는 동승자 필수, 일반 모드는 선택 (또는 동승자 없이).
"use client";
...
export function StartTripButton({
  ...
  // KIDS 모드면 helper 필수 — default 첫 번째 helper. 일반 모드는 "없이"가 default.
      ...
      <Dialog open={open} onOpenChange={(v) => !pending && setOpen(v)}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-2">
            <p className="text-foreground flex items-center gap-2 text-xs font-extrabold tracking-wide uppercase">
              <UserCheck className="h-3.5 w-3.5" />
              동승자 선택
            </p>

            {helpers.length === 0 ? (
              <div className="bg-muted text-muted-foreground rounded-md border p-3 text-xs">
                {isKids
              ...
              <div className="max-h-64 space-y-1.5 overflow-y-auto">
                {!isKids ? (
                  <label className="hover:bg-muted/40 flex cursor-pointer items-center gap-3 rounded-md border p-2.5">
                    <input
                      type="radio"
                      name="helper"
                ...
                {helpers.map((h) => (
                  <label
                    key={h.id}
                    className="hover:bg-muted/40 flex cursor-pointer items-center gap-3 rounded-md border p-2.5"
                  >
                    <input
                      type="radio"
                      name="helper"
